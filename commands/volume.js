module.exports.callback = async ({ client, message, args }) => {
	...
	if (!args[0] || isNaN(args[0]) || parseInt(args[0], 10) > 100 || parseInt(args[0], 10) < 1) {
		return message.reply(`Volume can only be changed between 1 and 100. Current Volume: ${serverQueue.volume}%`);
	}
  ...
  return message.channel.send(`Volume has been set to ${serverQueue.volume}%`);
};
